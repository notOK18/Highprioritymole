"""Drag-and-drop UI to find supplier molecules that appear in either reference list.

Drop your Supplier file in the first box, the High Monopoly list in the second,
and the Potential molecules list in the third, then click Find Common Molecules.
It writes a workbook with a 'Supplier' sheet and a 'Common' sheet (each match
tagged with which list it was found in). The logic lives in supplier_common.py.
"""
...
from supplier_common import compare_supplier  # noqa: E402
...
class App:
    def __init__(self, root):
        self.root = root
        root.title("Supplier Common Molecules")
        ...
        tk.Label(outer, text="Supplier Common Molecules", bg=BG, fg=TEXT,
                 font=("Helvetica", 22, "bold")).pack(anchor="w")
        tk.Label(outer,
                 text="Drop a supplier file and the two lists. Finds supplier molecules "
                      "that appear in EITHER list.",
                 bg=BG, fg=MUTED, font=("Helvetica", 11), wraplength=520,
                 justify="left").pack(anchor="w", pady=(2, 14))

        self.supplier = DropZone(self, outer, "1 · Supplier file",
                                 "The supplier's molecule / product list.", "sheet")
        self.hp = DropZone(self, outer, "2 · High Monopoly molecules",
                           "The high-priority, monopoly-competition list.", "monopoly")
        self.potential = DropZone(self, outer, "3 · Potential molecules",
        ...
        self.status = tk.Label(outer, text="Waiting for all three files…", bg=BG, fg=MUTED,
    ...
    def refresh(self):
        ready = bool(self.supplier.path and self.hp.path and self.potential.path)
    ...
    def run(self):
        ...
        args = (self.supplier.path, self.hp.path, self.potential.path,
                self.supplier.sheet(), self.hp.sheet(), self.potential.sheet())
    ...
    def _done(self, out, count):
        ...
        self.status.configure(
            text=f"✓ {count} supplier molecules appear in either list.\nSaved to: {out}\n"
                 f"Sheets: 'Supplier', 'Common'."
        )
